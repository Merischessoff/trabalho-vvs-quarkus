...
function background(){
     ...
     */
    let body = document.body.style.backgroundColor = '#BECCC3';
...
function show(){
    ...
    document.getElementById("result").innerHTML = name + " " + last_name;
}
...
function search(){
     ...
     */
    let searchField = document.querySelector("#name").value.toUpperCase();
    let resultado = document.getElementById("search");
    removeAllChildren(resultado);
    if (!searchField.length > 0)return;
    //resultado.innerHTML = data
    data.forEach(word => {
        if(word.name.substring(0, searchField.length).toUpperCase() == searchField){
            resultado.innerHTML += `<div>${word.name}</div>`};
